...
Write-Host "Current URL length: $($currentPhotoUrl.Length)"

if ($currentPhotoUrl -match '\.pdf') {
  Write-Host "Type: PDF (needs extraction)" -ForegroundColor Yellow
} else {
  Write-Host "Type: IMAGE (already extracted)" -ForegroundColor Green
}
...
if ($extractResponse.StatusCode -eq 200) {
  ...
  $newUrl = $result.profile_photo_url
  Write-Host "New URL: $($newUrl.Substring(0, 80))..."
} else {
  Write-Host "FAILED: Status $($extractResponse.StatusCode)" -ForegroundColor Red
}
