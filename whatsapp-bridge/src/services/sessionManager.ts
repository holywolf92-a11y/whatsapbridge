...
export class SessionManager {
  ...
  async start(): Promise<void> {
    ...
    // Railway 1 GB volume doesn't fill up with stale browser cache data.
    this.purgeChromeCache();
    setInterval(() => { this.purgeChromeCache(); }, 6 * 60 * 60 * 1000);
  }

  /**
   * Delete Chrome cache/blob_storage/GPUCache dirs from inside each
   * RemoteAuth session folder on the volume.  These are regenerated by Chrome
   * on next launch and can grow to hundreds of MB per session.
   */
  private purgeChromeCache(): void {
    const CACHE_SUBDIRS = ['Cache', 'Code Cache', 'GPUCache', 'blob_storage', 'DawnWebGPUCache'];
    for (const account of this.accounts) {
      if (!account.enabled) continue;
      const profileRoot = path.join(this.sessionDataPath, `RemoteAuth-${account.id}`, 'Default');
      for (const sub of CACHE_SUBDIRS) {
        const target = path.join(profileRoot, sub);
        try {
          if (fs.existsSync(target)) {
            fs.rmSync(target, { recursive: true, force: true });
            this.logger.info({ accountId: account.id, dir: sub }, 'Purged Chrome cache dir from volume');
          }
        } catch {
          // non-fatal
        }
  ...
  private async resetPersistedAuth(accountId: string): Promise<void> {
    ...
    const sessionDir = path.join(this.sessionDataPath, remoteAuthName);
    const sessionZip = path.join(this.sessionDataPath, `${remoteAuthName}.zip`);
