...
exports.startWhatsAppMonitor = startWhatsAppMonitor;
exports.stopWhatsAppMonitor = stopWhatsAppMonitor;
...
async function fetchBridgeStatus() {
    ...
    if (!res.ok)
        throw new Error(`Bridge /status returned ${res.status}`);
    const data = (await res.json());
    return data.sessions ?? [];
}
function canSendAlert(accountId) {
    const last = lastAlertSentAt.get(accountId);
    if (!last)
        return true;
    return Date.now() - last > ALERT_THROTTLE_MS;
}
async function sendDisconnectAlert(session, previousStatus) {
  ...
  `;
    ...
    try {
        await emailService_1.emailService.sendEmail({ to: ALERT_EMAIL, subject, html, text });
