...
exports.isPIIEncrypted = isPIIEncrypted;
exports.encryptPII = encryptPII;
exports.decryptPII = decryptPII;
exports.encryptCandidatePII = encryptCandidatePII;
exports.decryptCandidatePII = decryptCandidatePII;
const crypto = __importStar(require("crypto"));
const errorHandling_1 = require("./errorHandling");
const logger = (0, errorHandling_1.createLogger)('PIIEncryption');
...
function getKey() {
    ...
    if (!hex)
        return null;
    if (hex.length !== 64) {
        logger.warn('PII_ENCRYPTION_KEY is set but has wrong length (expected 64 hex chars = 32 bytes). ' +
...
function decryptPII(value) {
    if (!value)
        return value;
    if (!isPIIEncrypted(value))
        return value; // plaintext (legacy or unencrypted field)
    const key = getKey();
    if (!key) {
 ...
 */
function encryptCandidatePII(candidate) {
    return {
        ...candidate,
 ...
 */
function decryptCandidatePII(candidate) {
    return {
        ...candidate,
