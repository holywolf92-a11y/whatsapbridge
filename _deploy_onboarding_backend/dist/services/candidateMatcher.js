...
class CandidateMatcher {
    ...
    static async findCandidate(criteria) {
        ...
        // ── Phone ──────────────────────────────────────────────────────────────────
        if (criteria.phone) {
            const e164 = (0, candidateService_1.normalizePhoneE164)(criteria.phone);
            if (e164) {
        ...
        if (criteria.name) {
            ...
            if (!nameError && nameData && nameData.length > 0) {
                ...
                const nameMatches = (a, b, stdThreshold) => {
                    const sim = this.calculateSimilarity(a, b);
                    if (sim >= stdThreshold)
                        return true;
                    // Phonetic fallback: same soundex code and similarity ≥ 0.65
                    if (sim >= 0.65 && this.phoneticMatch(a, b))
                        return true;
                    return false;
                };
                // Name + DOB
                if (dobISO) {
                    const dobMatches = nameData.filter((c) => {
                ...
                if (normalizedFather) {
                    const fatherMatches = nameData.filter((c) => {
                        if (!c.father_name)
                            return false;
                        const cn = this.normalizeName(c.name || '');
                ...
                if (!hasNameSignal) {
                    const nameOnlyMatches = nameData.filter((c) => {
                        const cn = this.normalizeName(c.name || '');
        ...
        if (byCandidate.size > 1) {
            const conflictDetails = [...byCandidate.entries()].map(([cid, sigs]) => `${cid} via [${sigs.map(s => s.source).join(', ')}]`);
        ...
        if (matchedSignals.length > 1) {
            logger.info(`Probabilistic match: candidate=${matchedCandidateId}, signals=[${matchedSignals.map(s => s.source).join(', ')}], ` +
        ...
        this.persistMatchConfidence(matchedCandidateId, finalConfidence, matchSignals).catch((err) => logger.warn('Failed to persist match confidence (non-fatal)', { message: err.message }));
        return {
            candidateId: matchedCandidateId,
            matchedBy: primarySignal.source,
     ...
     */
    static async persistMatchConfidence(candidateId, confidence, signals) {
        const db = (0, database_1.supabaseAdminClient)();
        await db
            .from('candidates')
            .update({
     ...
     */
    static normalizePakistaniName(name) {
        const VARIANTS = [
            // Muhammad variants
     ...
     */
    static phoneticMatch(name1, name2) {
