...
async function fixCVExtractedPhotos() {
  ...
  try {
    ...
    const cvExtractedCandidates = candidates.filter((c: any) => {
    ...
    console.log('[Migration] Candidates to fix:', cvExtractedCandidates.map((c: any) => `${c.name} (${c.id})`));

    // Clear profile_photo_url for these candidates
    const candidateIds = cvExtractedCandidates.map((c: any) => c.id);
    ...
    return { 
      ...
      candidates: cvExtractedCandidates.map((c: any) => ({ id: c.id, name: c.name }))
    };
  } catch (err: any) {
