...
export async function trackAnalyticsController(req: Request, res: Response) {
  try {
    const db = supabaseAdminClient();
...
export async function listFeedbackController(req: Request, res: Response) {
  try {
    const db = supabaseAdminClient();
...
export async function getReviewStatsController(req: Request, res: Response) {
  try {
    const db = supabaseAdminClient();
    ...
    (feedbackData || []).forEach((row: { rating: number }) => {
      if (row.rating in ratingCounts) ratingCounts[row.rating]++;
    });
