import { DOCUMENT_CATEGORIES, type DocumentCategory } from '../config/documentCategories';
...
const PROFESSION_RULES: Array<{ role: string; keywords: string[] }> = [
  {
...
function cleanProfessionLabel(value: string): string {
  return value
    .trim()
    .replace(/\s+/g, ' ')
    .replace(/^[-:]+/, '')
...
function extractExperienceTitles(experience: unknown): string[] {
...
export function inferProfessionFromCvData(parsedCandidate: any): string | undefined {
  ...
  const experienceDescriptions = Array.isArray(parsedCandidate?.experience)
    ? parsedCandidate.experience
        .map((entry: any) => [entry?.description, ...(Array.isArray(entry?.responsibilities) ? entry.responsibilities : [])].filter(Boolean).join(' '))
        .filter(Boolean)
    : [];
...
export function inferProfessionFromExperienceCertificate(fileName: string, category?: DocumentCategory): string | undefined {
  if (category && category !== DOCUMENT_CATEGORIES.EXPERIENCE_CERTIFICATES) {
    return undefined;
  }
