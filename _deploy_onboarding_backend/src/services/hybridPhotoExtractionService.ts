 ...
 */
async function extractPhotoPythonParser(pdfBuffer: Buffer, attachmentId: string): Promise<Buffer | null> {
  try {
    if (!PARSER_URL) {
 ...
 */
export async function uploadExtractedPhotoToCandidatePhotos(
  candidateId: string,
  attachmentId: string,
  photoBuffer: Buffer
): Promise<{ bucket: string; storagePath: string; signedUrl: string }> {
 ...
 */
export async function extractProfilePhotoHybrid(
  candidateId: string,
  attachmentId: string,
  pdfBuffer: Buffer
): Promise<{
