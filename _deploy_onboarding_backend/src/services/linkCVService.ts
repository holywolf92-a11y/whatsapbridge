...
import { CandidateDocument } from './candidateDocumentService';
 ...
 */
export async function linkExistingCVFromInbox(candidateId: string): Promise<CandidateDocument> {
  ...
  if (existingCV) {
    ...
    return existingCV as CandidateDocument;
  }
  ...
  if (inboxError || !inboxCVs || inboxCVs.length === 0) {
    throw new AppError('No CV found in inbox attachments for this candidate', ErrorType.NOT_FOUND, 404);
  }
  ...
  const { data: newDocument, error: insertError } = await db
    ...
    .insert({
      ...
      verification_status: VERIFICATION_STATUS.VERIFIED, // CVs from inbox are already processed
  ...
  return newDocument as CandidateDocument;
}
