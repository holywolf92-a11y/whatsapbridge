 ...
 */
export function generateLegacyFilename(docType: string, timestamp?: number): string {
  const ts = timestamp || Date.now();
 ...
 */
export function extractDocInfoFromSplit(splitDoc: any): SplitDocInfo {
  return {
