...
function isVarcharOverflowError(err: any): boolean {
  ...
  if (code === '22001') return true;

  return (
    /value too long for type character varying\(\d+\)/i.test(message) ||
...
export function startCvParserWorker() {
  ...
  const worker = new Worker(
    'cv-parsing',
    async (job: Job) => {
      const { jobId, attachmentId, fileHash, force } = job.data as {
        jobId: string;
        attachmentId: string;
      ...
      try {
        const db = supabaseAdminClient();
        const updateAttachmentParsingStatus = async (
          status: 'processing' | 'extracted' | 'failed',
          extra?: { candidate_id?: string | null }
        ) => {
          const payload: Record<string, any> = { parsing_status: status };
        ...
        const setJobAndAttachmentStatus = async (
          status: 'processing' | 'extracted' | 'failed',
        ...
        const fileArrayBuffer = await (fileData as Blob).arrayBuffer();
        ...
        if (newCandidate?.id && mimeType !== 'application/pdf') {
          try {
            ...
            if (!existingCvErr && (!existingCvDoc || existingCvDoc.length === 0)) {
              ...
              const uploadErr = (upload as any)?.error;
              if (uploadErr) {
              ...
              const cvDocData: any = {
                ...
                detected_category: DOCUMENT_CATEGORIES.CV_RESUME,
                confidence: null,
            ...
            try {
              ...
              if (!hasLinkedErr && (!hasLinked || hasLinked.length === 0)) {
                ...
                const cvDocData: any = {
                  ...
                  storage_bucket: STORAGE_BUCKET,
                  storage_path: destPath,
                  file_name: generateDescriptiveFilename({ doc_type: 'cv_resume' }, newCandidate.name, timestamp),
                  mime_type: mimeType,
                  source: 'web',
                  status: 'received',
                  verification_status: VERIFICATION_STATUS.PENDING_AI,
        ...
        if (isVarcharOverflowError(err)) {
          ...
          if (attachmentUpdateError) {
            console.warn(
              `[CVParser] Failed to defuse attachment ${attachmentId} after varchar overflow:`,
          ...
          throw new UnrecoverableError(`Non-retryable DB varchar overflow: ${errMsg}`);
        }
